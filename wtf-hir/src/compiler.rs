...
fn compile_type_declaration(declaration: ast::Declaration, ast_types: &HashMap<String, ast::Declaration>, out: &mut HashMap<String, Type>) -> Type {
    ...
    let type_ = match declaration {
        ast::Declaration::Record(record) => {
            ...
            for field in record.fields {
                fields.insert(field.name, compile_type_annotation(field.type_annotation, ast_types, out));
            }
            Type::Record(fields)
        },
        ast::Declaration::Resource(resource) => {
            ...
            for method in resource.methods {
                methods.insert(method.name, FunctionSignature {
                    methods: method.parameters.iter().map(|param| compile_type_annotation(param.type_annotation, ast_types, out)),
...
fn compile_type_annotation(annotation: ast::TypeAnnotation, ast_types: &HashMap<String, ast::Declaration>, out: &mut HashMap<String, Type>) -> Type {
    match annotation {
        ...
        ast::TypeAnnotation::List(item) => Type::List(compile_type_annotation(item, ast_types, out)),
        ast::TypeAnnotation::Option(payload) => Type::Option(compile_type_annotation(payload, ast_types, out)),
        ...
        ast::TypeAnnotation::Tuple(fields) =>
            Type::Tuple(fields.into_iter().map(|field| compile_type_annotation(field, ast_types, out)).collect_vec()),
    }
}
