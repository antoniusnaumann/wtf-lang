...
fn compile_fun(declaration: &ast::FunctionDeclaration, ast_types: &HashMap<String, ast::Declaration>) -> Function {
    let parameters = declaration.parameters.iter().map(|param| (param.name.clone(), compile_type_annotation(&param.type_annotation, ast_types))).collect();
    ...
    let (expressions, body) = {
        ...
        for param in &declaration.parameters {
            ...
            visible.bind(param.name.to_string(), register(&mut expressions, &mut body, Expression::param(type_)));
        }
    
        let inner_body = compile_block(&declaration.body, &mut visible, &mut expressions);
        let expr = get(&expressions, inner_body);
        let ExpressionKind::Block { children, result } = expr.kind else { unreachable!() };
        body.extend(children);
        let body = register(&mut expressions, &mut vec![], Expression::block(body, result, expr.type_));
...
fn compile_block(block: &ast::Block, visible: &mut Visible, exprs: &mut Vec<Expression>) -> Id {
    ...
    register(exprs, &mut vec![], Expression::block(body, return_expr, return_type))
}
fn compile_statement(statement: &ast::Statement, visible: &mut Visible, exprs: &mut Vec<Expression>, body: &mut Vec<Id>) -> Id {
    match statement {
        ast::Statement::VariableDeclaration(variable_declaration) => {
            // TODO: allow uninitialized variables
            let value = compile_expression(variable_declaration.value.as_ref().unwrap(), visible, exprs, body);
        ...
        ast::Statement::ExpressionStatement(expression) => compile_expression(expression, visible, exprs, body),
        ...
        ast::Statement::ContinueStatement => {
            register(exprs, body, Expression::continue_())
        },
        ...
        ast::Statement::WhileStatement(while_statement) => {
            ...
            let condition = compile_expression(&while_statement.condition, visible, exprs, &mut inner_body);
            let none = register(exprs, &mut inner_body, Expression::none());
            let then_body = compile_block(&while_statement.body, visible, exprs);
            let else_body = none;
            let if_ = register(exprs, &mut inner_body, Expression::if_(
                condition,
                then_body,
                else_body
            ));
...
fn compile_expression(expression: &ast::Expression, visible: &mut Visible, exprs: &mut Vec<Expression>, body: &mut Vec<Id>) -> Id {
    match expression {
        ast::Expression::Literal(literal) => {
            match literal {
                ...
                ast::Literal::String(string) => register(exprs, body, Expression::string(string.clone())),
        ...
        ast::Expression::Identifier(name) => {
            visible.lookup(&name).expect(&format!("Variable {} is not defined.", name))
        },
        ast::Expression::BinaryExpression { left, operator, right } => {
            ...
            match operator {
                ...
                ast::BinaryOperator::GreaterThan => register(exprs, body, Expression::call("greater_than".to_string(), vec![left, right])),
